...
const EditProfilePopup = ({ onCancel, initialData }) => {
  ...
  const validateForm = () => {
    ...
    if (!phoneRegex.test(formData.phone)) {
      setError(
        'El número de teléfono debe comenzar con "+56" y tener al menos 9 dígitos',
      );
      return false;
    }
    ...
    try {
      const dataToSend = {
        ...
        ...(formData.profileImage.url && {
          profilePicture: formData.profileImage.url,
        }),
      };
        ...
        <div className="flex flex-col items-center mb-4">
          ...
          <div className="relative">
            ...
            {formData.profileImage.uploading && (
              <p className="text-xs text-yellow-600 absolute -bottom-5 left-0 right-0 text-center">
                Subiendo...
              </p>
            )}
            {formData.profileImage.error && (
              <p className="text-xs text-red-600 absolute -bottom-5 left-0 right-0 text-center">
                {formData.profileImage.error}
              </p>
            )}
          </div>
